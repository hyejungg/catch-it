# QA Checklist: MVP

## 목적
- CatchIt MVP의 핵심 플로우/리스크 항목을 수동 QA로 점검한다.

## 환경
- Chrome 최신 버전
- 확장 프로그램 빌드 후 로드(`dist`)
- 테스트 페이지(일반 웹사이트 2개 이상)
- Notion 연동 테스트용 Token / Database ID

## 필수 체크 항목

### 1) 드래그 수집
- 텍스트 3자 이상 드래그 시 팝오버(태그 입력 + 저장)가 표시된다.
- 3자 미만 드래그 시 팝오버가 표시되지 않는다.
- `Alt + Drag` 옵션 ON 상태에서는 Alt 없이 드래그 시 팝오버가 표시되지 않는다.
- 드래그된 선택 영역의 하이라이트 색상이 `rose` 계열로 표시된다.
- 팝오버 태그 입력칸에 `태그1, 태그2` 형태로 입력 후 저장하면 태그가 저장된다.
- 태그 입력 중에도 선택 하이라이트가 유지되어야 한다.
- 태그 입력 시작 전/후 하이라이트 영역이 갑자기 과도하게 줄어들지 않아야 한다.
- 저장 클릭 시 항목이 Dashboard 목록에 추가된다.
- 팝오버 외부 영역 클릭 시 팝오버가 닫히고 저장되지 않는다.

### 2) 대시보드 목록/검색
- 저장된 항목이 최신순으로 표시된다.
- 검색어로 `text/title/url/tag` 포함 검색이 동작한다.
- 복사 버튼이 텍스트를 클립보드에 복사한다.
- 원문 열기 버튼이 새 탭으로 URL을 연다.
- 삭제 버튼이 항목을 제거하고 목록에 즉시 반영된다.
- 확장 아이콘으로 열리는 UI 폭이 충분해 주요 컨텐츠가 잘리지 않는다.
  - Side Panel 모드 사용 시: 기본 진입이 side panel에서 동작한다.
  - Popup 모드 유지 시: 텍스트/버튼/상태 배지가 겹치지 않는다.

### 3) Settings 저장
- `notionToken`, `notionDbId` 입력값이 저장/재진입 후 유지된다.
- `autoSync`, `requireAlt` 토글이 저장/재진입 후 유지된다.
- Notion 연동 카드에 Token/Database ID 획득 방법 안내 문구와 링크가 노출된다.
- Notion 연동 카드에 템플릿 복제 링크가 노출된다.
- 타이틀 구조가 요구사항과 일치한다.
  - 메인 타이틀: `CatchIt - 웹 텍스트 수집기`
  - 서브 타이틀: `Dashboards` / `Settings`

### 4) Notion 동기화
- 미동기화 기본 상태는 `ready`로 표시되고, 동기화 실패 전에는 `failed`가 표시되지 않는다.
- `autoSync` ON 상태에서 저장 시 Notion 페이지가 생성된다.
- `Sync Now` 클릭 시 미동기화 항목이 배치 동기화된다.
- 성공 시 `notion.status = sync`, `pageId`, `syncedAt`이 반영된다.
- 설정 누락 시 `failed` 상태와 오류 메시지가 기록된다.
- 401/403/429 응답 시 오류 메시지가 구분되어 기록된다.
- Notion DB 속성명이 `Name/Quote/URL/SavedAt/Tags`와 달라도 타입(title/rich_text/url/date/multi_select) 기준으로 동기화된다.
- `연동 확인(Test Connection)` 버튼으로 설정값 유효성을 즉시 확인할 수 있다.
- `Notion DB 바로가기(Open Notion)` 버튼으로 대상 데이터베이스 페이지가 열린다.
- `연동 확인` 실행 후 성공/실패 상태 메시지가 UI에 표시된다.
- `연동 확인` 성공 시 저장된 항목의 `pageId`가 Notion DB에 존재하면 `sync`, 없으면 `ready`로 재평가된다.
- `Database ID`를 변경하면 기존 하이라이트의 상태 배지가 일괄 `ready`로 초기화된다.

### 5) 상태 갱신/반영
- content-script 저장 후 Dashboard가 새로고침 없이 갱신된다.
- Sync 상태 변경이 목록 상태 배지에 반영된다.

## 회귀 리스크 체크
- 토큰/DB ID가 콘솔 로그에 노출되지 않는다.
- 팝오버가 페이지 클릭 시 정상적으로 닫힌다.
- 대량(예: 100개+) 데이터에서도 목록/검색 응답이 과도하게 느리지 않다.

## 결과 기록 템플릿
- 실행 날짜:
- 실행자:
- 브라우저 버전:
- 결과 요약: pass / partial / fail
- 실패 항목:
- 재현 절차:
- 비고:

## 최근 실행 결과 (Playwright)
- 실행 날짜: 2026-02-28
- 실행자: Codex (Playwright MCP)
- 대상 커밋: `085cc73`
- 브라우저/환경: Playwright Chromium + `vite preview` (`http://127.0.0.1:4173`)
- 결과 요약: `partial`

### 확인 완료
- Dashboard 기본 진입 및 헤더 렌더링 확인
  - `CatchIt - 웹 텍스트 수집기`
  - `Dashboards` -> `Settings` 전환
- Settings 화면 요소 렌더링 확인
  - Token/Database ID 입력 필드
  - `연동 확인`, `Notion DB 바로가기` 버튼
  - 템플릿 복제 링크 노출
- Notion API 접근성 확인 (Playwright `page.request`)
  - DB ID `66a24f69b4728257835f01936989cce1`: `GET /v1/databases/{id}` 200, `POST /v1/databases/{id}/query` 200
  - DB ID `31524f69b47280bc8ba8da1da05f968e`: `GET /v1/databases/{id}` 200, `POST /v1/databases/{id}/query` 200

### 관찰 사항
- 두 DB 모두 query 결과 건수 `0`
- 이 경우 `연동 확인` 재평가 시 `sync` 매칭 대상이 없어 전체가 `ready`로 보일 수 있음

### 제한 사항
- Playwright 환경에서는 Chrome 확장(runtime/background) end-to-end를 완전 대체하지 못함
- `연동 확인` 버튼 클릭 후 실제 background 로직 실행 검증은 Chrome 확장 로드 환경에서 별도 수동 QA 필요
